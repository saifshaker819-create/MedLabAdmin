...
export default async function handler(req, res) {
  ...
  if (!token) return res.status(500).json({ ok: false, description: 'TG_TOKEN غير مضبوط بإعدادات Vercel' });

  const body = typeof req.body === 'string' ? JSON.parse(req.body || '{}') : (req.body || {});
  const chat_id = body.chat_id || process.env.TG_CHAT;
  if (!chat_id) return res.status(400).json({ ok: false, description: 'chat_id ناقص' });
